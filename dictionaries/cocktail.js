(() => {
  ...
  "exactEntries": [
    ...
    [
      " 是 cocktail 的增强版，包含可选后端 Server Plugin，可进一步优化启动、角色列表、settings/chat 保存等性能路径。",
      " 는 cocktail의 확장판으로, 선택적 백엔드 Server Plugin을 포함합니다. 시작 로딩, 캐릭터 목록, settings/chat 저장 등의 성능 경로를 한층 더 최적화합니다."
    ],
    [
      "安装后会新增一个独立前端扩展；进入 鸡尾酒+ 面板后，可按脚本助手安装后端插件。",
      "설치하면 별도의 프론트엔드 확장이 추가됩니다. 칵테일+ 패널에 들어가면 안내에 따라 백엔드 플러그인을 설치할 수 있습니다."
    ],
    [
      "国外下载（GitHub）",
      "해외 다운로드 (GitHub)"
    ...
    [
      "启用聊天渲染优化（总开关）",
      "채팅 렌더링 최적화 활성화 (전체 스위치)"
    ],
    [
      "- “启用聊天渲染优化”：总开关。关闭后，本模块不会应用分页、加载更多、代码高亮、隐藏代码块和手势相关优化。",
      "- \"채팅 렌더링 최적화 활성화\": 전체 스위치입니다. 끄면 이 모듈의 페이지 분할, 추가 로드, 코드 강조, 코드 블록 숨기기, 제스처 관련 최적화가 적용되지 않습니다."
    ],
    [
      "- “隐藏代码块”：默认关闭。开启后会显示占位符；点击占位符可展开原代码块。",
      "- \"코드 블록 숨기기\": 기본값은 꺼짐입니다. 켜면 자리 표시자가 표시되며, 자리 표시자를 클릭하면 원래 코드 블록이 펼쳐집니다."
    ],
    [
      "对 PromptManager 预设条目列表启用拖拽排序优化。",
      "PromptManager 프리셋 항목 목록에 드래그 정렬 최적화를 적용합니다."
    ],
    [
      "推荐：指示线拖拽（拖动中不重排条目，松手一次性移动并保存），更流畅。",
      "권장: 가이드라인 드래그 (드래그 중에는 항목을 재정렬하지 않고, 마우스를 놓을 때 한 번에 이동 및 저장). 더 부드럽게 작동합니다."
    ],
    [
    ...
    [
      ...
      "'기본 sortable' 엔진에서만 적용: 드래그 helper를 body에 추가하여 목록의 overflow에 잘리는 현상을 방지합니다."
    ],
    [
    ...
    [
      ...
      "목록의 상하 가장자리 근처로 드래그하면 자동으로 스크롤되어, 항목을 아주 먼 위치로 옮기기 쉽습니다."
    ],
    [
      "拖拽贴近边缘时的最大滚动速度（每帧像素）。",
    ...
    [
      "关闭 backdrop-filter 可明显降低 GPU 压力（尤其是内容很多的抽屉面板）。",
      "backdrop-filter를 끄면 GPU 부하를 크게 줄일 수 있습니다 (특히 내용이 많은 드로어 패널에서)."
    ],
    [
    ...
    [
      "- “启用分页渲染”：默认开启。关闭后会尽量一次性渲染全部消息（建议点“应用并重载聊天”）。",
      "- \"페이지네이션 렌더링 활성화\": 기본적으로 켜져 있습니다. 비활성화하면 최대한 모든 메시지를 한 번에 렌더링합니다('적용 및 채팅 새로고침'를 클릭하는 것을 권장합니다)."
    ],
    [
      "- “隐藏代码块”：不直接显示",
    ...
    [
      "- “启用分页渲染”：默认开启.关闭后会尽量一次性渲染全部消息（建议点“应用并重载聊天”）.",
      "- \"페이지네이션 렌더링 활성화\": 기본적으로 켜져 있습니다. 끄면 가능한 한 모든 메시지를 한 번에 렌더링합니다. (권장: “적용 및 채팅 새로고침” 클릭)"
    ],
    [
    ...
    [
      ...
      "- 월드 인포 항목 드로어: 클릭 시 먼저 \"자리 잡기 + 애니메이션 + 로딩\"을 수행하고, 애니메이션 종료 후 무거운 콘텐츠를 초기화하여 프레임 드랍을 방지합니다."
    ],
    [
    ...
    [
      ...
      "- PromptManager 목록 스위치는 더 이상 토큰 계산 완료를 기다리지 않습니다(토큰 통계는 백그라운드에서 업데이트됨)."
    ],
    [
      "- 预设条目拖拽优化与 WorldInfo 拖拽优化对齐：支持“指示线拖拽 / 原生 sortable”两种引擎。",
      "- 프리셋 항목 드래그 앤 드롭 최적화를 월드 인포 드래그 앤 드롭 최적화와 동일하게 맞춤: '가이드라인 드래그 / 기본 sortable' 두 가지 엔진을 지원합니다."
    ],
    [
      "- 本插件在“正则面板展开期间”拦截这些刷新；当面板收起/隐藏时才统一刷新一次。",
      "- 이 확장은 \"정규식 패널이 열려있는 동안\" 새로고침을 차단하고, 패널이 닫히거나 숨겨질 때 한 번에 새로고침합니다."
    ],
    [
      "- 仅对",
      "- 다음 항목에만 적용: "
    ...
    [
      ...
      "- 항목 에디터 캐시: 같은 월드 인포 내에서 열었던 항목 에디터를 자동 캐시합니다. 월드 인포를 전환하면 캐시가 자동 초기화됩니다."
    ],
    [
    ...
    [
      ...
      "- 태번 코어는 월드 인포 사용자 정의 정렬을 위해 jQuery UI sortable을 사용합니다."
    ],
    [
      "${0}/${1} 条消息",
      "${0}/${1} 메시지"
    ...
    [
      ...
      "[태번 헬퍼] 캐릭터 데이터 로드 실패. 빈 데이터를 사용합니다."
    ],
    [
      "[酒馆助手]读取聊天数据失败, 将使用空数据",
      "[태번 헬퍼] 채팅 데이터 로드 실패. 빈 데이터를 사용합니다."
    ],
    [
      "[酒馆助手]读取预设数据失败, 将使用空数据",
      "[태번 헬퍼] 프리셋 데이터 로드 실패. 빈 데이터를 사용합니다."
    ],
    [
      "[酒馆助手]读取全局数据失败, 将使用空数据",
      "[태번 헬퍼] 글로벌 데이터 로드 실패. 빈 데이터를 사용합니다."
    ],
    [
      "[酒馆助手]连接实时监听功能出错, 尝试重连...",
      "[태번 헬퍼] 실시간 모니터링 연결 오류. 재연결 시도 중..."
    ],
    [
      "[酒馆助手]实时监听器断开连接",
      "[태번 헬퍼] 실시간 리스너 연결이 끊어졌습니다."
    ],
    [
      "[酒馆助手]迁移旧数据失败, 将使用空数据",
      "[태번 헬퍼] 구 데이터 마이그레이션 실패. 빈 데이터를 사용합니다."
    ],
    [
      "<div>更新日志加载中...</div>",
      "<div>업데이트 로그를 불러오는 중...</div>"
    ],
    [
      "API 错误",
      "API 오류"
    ...
    [
      "UI 动画与抽屉展开优化",
      "UI 애니메이션 및 드로어 전개 최적화"
    ],
    [
      "WorldInfo 子面板展开优化（替换 slideToggle）",
      "월드 인포 하위 패널 전개 최적화 (slideToggle 대체)"
    ],
    [
      "WorldInfo 条目列表 content-visibility（实验）",
      "월드 인포 항목 목록 content-visibility (실험적)"
    ],
    [
    ...
    [
      "启用 UI 动画与展开优化",
      "UI 애니메이션 및 전개 최적화 활성화"
    ],
    [
      "启用 WorldInfo 拖拽排序优化（自定义排序）",
      "월드 인포 드래그 정렬 최적화 활성화 (사용자 정의 정렬)"
    ],
    [
      "启用：保存中仍可切换对话",
      "활성화: 저장 중에도 대화 전환 허용"
    ...
    [
      "本扩展免费使用，禁止任何形式的商业用途",
      "이 확장은 무료이며, 모든 형태의 상업적 이용을 금지합니다."
    ],
    [
    ...
    [
      "使用 Blob URL 渲染前端界面，更方便 f12 开发者工具调试；但某些浏览器可能不支持",
      "Blob URL로 프론트엔드를 렌더링하여 F12 디버깅을 돕습니다. (일부 브라우저는 이를 지원하지 않을 수 있습니다.)"
    ],
    [
      "使用方法",
      "사용법"
    ...
    [
      ...
      "설명: 이 확장은 window.load 대기 시간을 없앨 수는 없지만, 메인 화면을 더 빨리 보여주고 이후 API 대기 시간을 줄여 줍니다."
    ],
    [
    ...
    [
      "开启后，始终不渲染已经被设置为AI不可见的楼层，且不计入渲染深度设定的楼层数",
      "활성화하면, AI에게 보이지 않도록 설정된 메시지는 항상 렌더링하지 않으며, 렌더링 깊이 설정에 따른 메시지 수에도 포함되지 않습니다."
    ],
    [
      "成功导入脚本 '${0}'",
      "스크립트 '${0}' 가져오기 성공"
    ...
    [
      "世界书强制用推荐的全局设置",
      "월드 인포: 권장 글로벌 설정 강제 적용"
    ],
    [
      "世界书强制自定义排序",
      "월드 인포: 사용자 정의 정렬 강제 적용"
    ],
    [
      "世界书繁简互换: 一键将繁体/简体世界书翻译成简体/繁体",
      "월드 인포 번체/간체 변환: 번체/간체 월드 인포를 한 번에 번역"
    ],
    [
      "消息",
      "메시지"
    ...
    [
      "酒馆助手更新成功, 准备刷新页面以生效...",
      "태번 헬퍼 업데이트 성공. 적용을 위해 페이지를 새로고침합니다..."
    ],
    [
      "酒馆助手已是最新版本, 无需更新",
      "태번 헬퍼가 이미 최신 버전입니다."
    ...
    [
      ...
      "태그화: 월드 인포, 프리셋, 링크 설정에 따라 정규식/프롬프트 항목을 자동 제어"
    ],
    [
      "下载宏参考文件",
      "매크로 참고 파일 다운로드"
    ...
    [
      ...
      "월드 인포(#WorldInfo) 범위 내의 펼치기 목록에만 적용하여 다른 패널에 영향을 주지 않도록 합니다."
    ],
    [
      "扩展信息",
      "확장 정보"
    ...
    [
      ...
      "프리셋 드래그 앤 드롭 및 패널 최적화 활성화(UI를 먼저 전환하고, 백그라운드에서 통계를 보완함)"
    ],
    [
      "后台合并延迟(ms)",
      "백그라운드 병합 지연 (ms)"
    ...
    [
      "隐藏“额外匹配来源”",
      "'Additional Matching Sources' 숨기기"
    ],
    [
      "隐藏 主要关键字",
      "'기본 키워드' 숨기기"
    ...
    [
      "隐藏 绑定到角色或标签",
      "'Filter to Characters or Tags' 숨기기"
    ],
    [
      "隐藏 筛选生成触发器",
      "'Filter to Generation Triggers' 숨기기"
    ],
    [
      "隐藏 扫描深度",
      "'스캔 깊이' 숨기기"
    ...
    [
      ...
      "하단 구버전 컨트롤 숨기기 (WIEntryBottomControls)"
    ],
    [
      "Debug log",
      "디버그 로그"
    ...
    [
      "- 预设条目拖拽优化与 WorldInfo 拖拽优化对齐：支持“指示线拖拽 / 原生 sortable”两种引擎.",
      "- 프리셋 항목 드래그 최적화가 월드 인포 드래그 최적화와 동일하게 맞춰졌습니다: \"가이드라인 드래그 / 기본 sortable\" 두 가지 엔진을 지원합니다."
    ]
  ],
  "overrideEntries": [
    [
    ...
    [
      "是 cocktail 的增强版扩展，包含前端扩展和可选后端 Server Plugin。后端插件用于更深度优化，需要安装后重启 SillyTavern 才能生效。",
      "는 칵테일의 향상된 확장판으로, 프론트엔드 확장 및 선택적 백엔드 서버 플러그인을 포함합니다. 백엔드 플러그인은 더욱 심층적인 최적화에 사용되며, 설치 후 태번을 다시 시작해야 적용됩니다."
    ],
    [
      "状态：未检测到",
      "상태: 감지되지 않음"
    ...
    [
      ...
      "해외 다운로드(GitHub)를 통해 칵테일+ 설치를 진행합니다: https://github.com/Lianues/cocktail-plus 칵테일+에는 선택 사항인 백엔드 서버 플러그인이 포함되어 있습니다. 프론트엔드 확장을 설치한 후, 칵테일+ 패널에 들어가 안내에 따라 백엔드 플러그인을 설치하고 태번을 다시 시작해 주세요."
    ],
    [
      ...
      "국내 다운로드(Gitee)를 통해 칵테일+ 설치를 진행합니다: https://gitee.com/lianues/cocktail-plus 칵테일+에는 선택 사항인 백엔드 서버 플러그인이 포함되어 있습니다. 프론트엔드 확장을 설치한 후, 칵테일+ 패널에 들어가 안내에 따라 백엔드 플러그인을 설치하고 태번을 다시 시작해 주세요."
    ],
    [
      "开始安装",
      "설치 시작"
    ...
    [
      ...
      "권장: 가이드라인 드래그 (드래그 중에는 항목을 재정렬하지 않고, 마우스를 놓을 때 한 번에 이동 및 저장). 항목 UI가 복잡할 때 훨씬 부드럽게 작동합니다."
    ],
    [
    ...
    [
      ...
      "- 이 모듈은 드래그 방식을 '가이드라인 드래그'로 변경합니다: 드래그 도중에는 목록이 실시간으로 재구성되지 않으며, 마우스를 놓는 순간 한 번에 이동한 뒤 기존의 정지(Stop) 콜백을 호출하여 저장합니다."
    ],
    [
    ...
    [
      "强制使用推荐的世界书全局设置",
      "권장 월드 인포 전역 설정 강제 적용"
    ],
    [
      "导出角色卡时始终携带最新世界书",
      "캐릭터 카드 내보내기 시 최신 월드 인포 항상 포함"
    ],
    [
      "使用[替换/更新角色卡]功能时更新世界书",
      "[캐릭터 카드 교체/업데이트] 기능 사용 시 월드 인포 업데이트"
    ],
    [
      "取消前端代码高亮",
      "프론트엔드 코드 하이라이팅 비활성화"
    ...
    [
      "第 ${normalized_message_id} 楼",
      "${normalized_message_id}번째"
    ],
    [
      "酒馆助手脚本-${to_export.name}.json",
      "태번 헬퍼 스크립트-${to_export.name}.json"
    ],
    [
      "重命名失败",
      "이름 변경 실패"
    ...
    [
      ...
      "[태번 헬퍼] 캐릭터 카드 데이터 로드 실패. 빈 데이터를 사용합니다."
    ],
    [
      "[酒馆助手]连接实时监听器出错, 尝试重连...",
      "[태번 헬퍼] 실시간 리스너 연결 오류. 재연결 시도 중..."
    ],
    [
      "后端：未安装/未启用；SW：未注册 / 未接管当前页",
      "백엔드: 설치되지 않음/활성화되지 않음; SW: 등록되지 않음 / 현재 페이지를 제어하지 않음"
    ],
    [
      "后端：可用；SW：已注册 / 已接管",
      "백엔드: 사용 가능; SW: 등록됨 / 제어됨"
    ],
    [
      "Early Bridge：未安装；注入位置：",
      "Early Bridge: 설치되지 않음; 삽입 위치:"
    ],
    [
      "Early Bridge：已安装（最新）；注入位置：",
      "Early Bridge: 설치됨 (최신); 주입 위치:"
    ],
    [
      "通过 Service Worker + Early Bridge + 后端 Server Plugin 优化 SillyTavern 原始接口；当前优化",
      "Service Worker + Early Bridge + 백엔드 Server Plugin을 통해 태번 원본 인터페이스를 최적화합니다. 최적화 완료:"
    ],
    [
      "注册/更新 SW",
      "SW 등록/업데이트"
    ...
    [
      "更新检查（后端扩展和前端扩展更新是独立的，需要分别进行更新）",
      "업데이트 확인 (백엔드 확장과 프론트엔드 확장의 업데이트는 독립적이므로, 각각 개별적으로 업데이트해야 합니다)"
    ],
    [
      "远端：",
      "원격:"
    ...
    [
      ...
      "(자동 확인 시 GitHub를 먼저 확인하고, 실패 시 Gitee 확인)"
    ],
    [
      "前端扩展：当前 0.1.15，远端 0.1.15，已是最新<br>后端插件：当前 -，远端 0.1.15，已是最新",
      "프론트엔드 확장: 현재 0.1.15, 원격 0.1.15, 최신 버전입니다.<br>백엔드 플러그인: 현재 -, 원격 0.1.15, 최신 버전입니다."
    ],
    [
      "上次检查：2026. 6. 8. 오후 2:32:47；前端已跳过版本：-",
      "마지막 확인: 2026. 6. 8. 오후 2:32:47; 프론트엔드가 건너뛴 버전: -"
    ],
    [
      "检查更新",
      "업데이트 확인"
    ...
    [
      "复制对应系统命令到运行 SillyTavern 的机器终端中执行；命令直接从 GitHub/Gitee 下载脚本，不经过酒馆本地网页地址，因此不受酒馆登录验证影响。脚本会定位 ",
      "해당하는 시스템 명령을 복사하여 태번이 실행 중인 기기의 터미널에서 실행하세요. 명령은 태번 로컬 웹페이지 주소를 거치지 않고 GitHub/Gitee에서 스크립트를 직접 다운로드하므로 태번 로그인 인증의 영향을 받지 않습니다. 스크립트는 "
    ],
    [
      "，并可安装/删除后端插件、维护后端 ",
      "을 찾아 백엔드 플러그인 설치/제거, 백엔드 "
    ],
    [
      "、开关 ",
      " 유지 관리, "
    ...
    [
      " 或重启酒馆。",
      " 켜기/끄기 또는 태번 재시작을 수행할 수 있습니다."
    ],
    [
      "下载 cocktail-plus 后端脚本助手失败：请检查 GitHub/Gitee 网络。",
      "cocktail-plus 백엔드 스크립트 도우미 다운로드 실패: GitHub/Gitee 네트워크를 확인하세요."
    ],
    [
      "复制 Windows 命令",
      "Windows 명령 복사"
    ...
    [
      "ASYNC-MISS 后缓存就绪自动刷新角色列表",
      "ASYNC-MISS 후 캐시 준비 완료 시 자동으로 캐릭터 목록 새로고침"
    ],
    [
      "启动后自动异步检查 GitHub 更新",
      "시작 후 자동으로 비동기적으로 GitHub 업데이트 확인"
    ],
    [
      "缓存状态",
      "캐시 상태"
    ...
    [
      "暂无 settings/get 优化状态。",
      "현재 settings/get 최적화 상태가 없습니다."
    ],
    [
      "settings/save 优化状态",
      "settings/save 최적화 상태"
    ],
    [
      "暂无 settings/save 优化状态。",
      "현재 settings/save 최적화 상태가 없습니다."
    ],
    [
      "chat/save 优化状态",
      "chat/save 최적화 상태"
    ...
    [
      ...
      "fast settings/get은 원래의 응답 구조를 유지하며, 실제 Kobold/NovelAI/TextGen/OpenAI 프리셋, themes, instruct/context/sysprompt/reasoning 등의 필드를 반환합니다. 최적화 사항은 디렉토리 병렬 읽기와 메모리 캐시 활용이며, 더 이상 큰 필드에 대한 필터링을 수행하지 않습니다."
    ],
    [
      "工作方式：Early Bridge / Service Worker 捕获",
      "작동 방식: Early Bridge / Service Worker가"
    ],
    [
      "作为基线，保存时将",
      "을 기준선으로 포착하며, 저장 시"
    ...
    [
      "改为 no-op hash 或深层 JSON patch；冲突/失败自动回退原始完整保存。",
      "를 no-op 해시 또는 깊은 JSON 패치로 변경합니다. 충돌 및 실패 시에는 원본 전체 저장 방식으로 자동 fallback합니다."
    ],
    [
      "工作方式：Early Bridge 捕获",
      "작동 방식: Early Bridge가"
    ],
    [
      "作为基线，保存时将完整聊天上传改为 no-op hash 或聊天数组 patch；后端以内存缓存和文件 stat 校验当前 JSONL，冲突/失败自动回退原始保存。",
      "을 기준선으로 포착하며, 저장 시 전체 채팅 업로드 방식을 no-op 해시 또는 채팅 배열 패치로 변경합니다. 백엔드는 메모리 캐시와 파일 stat을 통해 현재 JSONL을 검증하며, 충돌 및 실패 시에는 원본 저장 방식으로 자동 fallback합니다."
    ]
  ],
  "regexRules": [{"replace":"서브패널 렌더링 실패: $1 (콘솔 확인)"},{"replace":"현재 버전: $1\n최신 버전: $2\n지금 업데이트하시겠습니까?"},{"replace":"현재 버전: $1"},{"replace":"최신 버전: $1"},{"replace":"최신: Ver $1"},{"replace":"선택한 $1개 스크립트를 이동하시겠습니까?"},{"replace":"선택한 $1개 스크립트를 삭제하시겠습니까?"}]
};
})();
