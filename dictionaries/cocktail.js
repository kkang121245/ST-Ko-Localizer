(() => {
  ...
  "exactEntries": [
    ...
    [
      ...
      "- \"페이징 렌더링 활성화\": 기본적으로 켜져 있습니다. 비활성화하면 최대한 모든 메시지를 한 번에 렌더링합니다('적용 및 채팅 다시 로드'를 클릭하는 것을 권장합니다)."
    ],
    [
      "- “隐藏代码块”：不直接显示",
    ...
    [
      ...
      "- 프리셋 항목 드래그 앤 드롭 최적화를 월드인포 드래그 앤 드롭 최적화와 동일하게 맞춤: '표시선 드래그 / 기본 sortable' 두 가지 엔진을 지원합니다."
    ],
    [
  ...
  "overrideEntries": [
    ...
    [
      ...
      "는 칵테일의 향상된 확장판으로, 프론트엔드 확장 및 선택적 백엔드 서버 플러그인을 포함합니다. 백엔드 플러그인은 더욱 심층적인 최적화에 사용되며, 설치 후 실리태번을 다시 시작해야 적용됩니다."
    ],
    [
      "状态：未检测到",
      "상태: 감지되지 않음"
    ...
    ["预热并等待", "예열 및 대기"],
    ["清空缓存", "캐시 비우기"],
    ["刷新 Early 状态", "Early 상태 새로고침"],
    ["安装/更新 Early Bridge", "Early Bridge 설치/업데이트"],
    ["卸载 Early Bridge", "Early Bridge 제거"],
    ["更新检查（后端扩展和前端扩展更新是独立的，需要分别进行更新）", "업데이트 확인 (백엔드 확장과 프론트엔드 확장의 업데이트는 독립적이므로, 각각 개별적으로 업데이트해야 합니다)"],
    ["远端：", "원격:"],
    ["（自动检查时先 GitHub，失败后 Gitee）", "(자동 확인 시 GitHub를 먼저 확인하고, 실패 시 Gitee 확인)"],
    ["前端扩展：当前 0.1.15，远端 0.1.15，已是最新<br>后端插件：当前 -，远端 0.1.15，已是最新", "프론트엔드 확장: 현재 0.1.15, 원격 0.1.15, 최신 버전입니다.<br>백엔드 플러그인: 현재 -, 원격 0.1.15, 최신 버전입니다."],
    ["上次检查：2026. 6. 8. 오후 2:32:47；前端已跳过版本：-", "마지막 확인: 2026. 6. 8. 오후 2:32:47; 프론트엔드가 건너뛴 버전: -"],
    ["检查更新", "업데이트 확인"],
    ["更新前端", "프론트엔드 업데이트"],
    ["后端插件脚本助手", "백엔드 플러그인 스크립트 도우미"],    
    [
      "复制对应系统命令到运行 SillyTavern 的机器终端中执行；命令直接从 GitHub/Gitee 下载脚本，不经过酒馆本地网页地址，因此不受酒馆登录验证影响。脚本会定位 ",
      "해당하는 시스템 명령을 복사하여 실리태번이 실행 중인 기기의 터미널에서 실행하세요. 명령은 태번 로컬 웹페이지 주소를 거치지 않고 GitHub/Gitee에서 스크립트를 직접 다운로드하므로 태번 로그인 인증의 영향을 받지 않습니다. 스크립트는 "
    ],
    [
      "，并可安装/删除后端插件、维护后端 ",
    ...
    ["下载 cocktail-plus 后端脚本助手失败：请检查 GitHub/Gitee 网络。", "cocktail-plus 백엔드 스크립트 도우미 다운로드 실패: GitHub/Gitee 네트워크를 확인하세요."],
    ["复制 Windows 命令", "Windows 명령 복사"],
    ["复制 Linux/macOS 命令", "Linux/macOS 명령 복사"],
    ["已复制", "복사 완료"],
    ["前端本地选项", "프론트엔드 로컬 옵션"],
    ["后端可用时自动注册 SW", "백엔드 사용 가능 시 자동으로 SW 등록"],
    ["后端可用时自动预热", "백엔드 사용 가능 시 자동으로 예열"],
    ["ASYNC-MISS 后缓存就绪自动刷新角色列表", "ASYNC-MISS 후 캐시 준비 완료 시 자동으로 캐릭터 목록 새로고침"],
    ["启动后自动异步检查 GitHub 更新", "시작 후 자동으로 비동기적으로 GitHub 업데이트 확인"],
    ["缓存状态", "캐시 상태"],
    ["暂无缓存状态。", "현재 캐시 상태가 없습니다."],
    ["settings/get 优化状态", "settings/get 최적화 상태"],
    ["暂无 settings/get 优化状态。", "현재 settings/get 최적화 상태가 없습니다."],
